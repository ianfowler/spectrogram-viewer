...
Spectrogram.prototype.setupVisual = function () {
  ...
  let maxFrequency =
    ...
    this.getBinFrequency(this.analyser.frequencyBinCount);
  ...
  this.maxCount =
    (this.context.sampleRate / this.sampleRate) * this.buffer.duration;
  ...
  this.yScale = d3.scaleLinear().domain(this.freqRange).range([this.height, 0]);
...
Spectrogram.prototype.showProgress = function () {
  if (this.isPlaying && this.isLoaded) {
    ...
    if (
      this.curDuration >= this.buffer.duration ||
      this.curDuration >= this.endTime
    ) {
...
Spectrogram.prototype.play = function () {
  ...
  let startIndex =
    Math.floor(
      (this.xScale.domain()[0] / this.timeRange[1]) * this.data.length
    ) || 0;
  let endIndex =
    Math.floor(
      (this.xScale.domain()[1] / this.timeRange[1]) * this.data.length
    ) - 1 || this.data.length;
  let startTime = 0;
...
Spectrogram.prototype.pauseResume = function () {
  ...
  if (this.isPlaying) {
    ...
    this.progressLine.transition().duration(0);
...
Spectrogram.prototype.draw = function () {
  ...
  let startIndex =
    Math.floor(
      (that.xScale.domain()[0] / this.timeRange[1]) * this.data.length
    ) || 0;
  let endIndex =
    Math.floor(
      (that.xScale.domain()[1] / this.timeRange[1]) * this.data.length
    ) || this.data.length;
  ...
  this.dotHeight =
    (this.height / this.analyser.frequencyBinCount) *
      (this.freqRange[1] / this.yScale.domain()[1]) +
    1;
